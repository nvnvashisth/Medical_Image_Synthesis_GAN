...
# Test - simple model
    ...
    def trainSimpleModel(self):
        ...
        for epoch in range(epochs):
            ...
            self.G_A2B.fit(x=self.A_train, y=self.B_train, epochs=1, batch_size=4)
            self.G_B2A.fit(x=self.B_train, y=self.A_train, epochs=1, batch_size=4)
            #loss = self.G_A2B.train_on_batch(x=real_A, y=real_B)
            #print('loss: ', loss)
            synthetic_image_A = self.G_B2A.predict(real_B, batch_size=4)
            synthetic_image_B = self.G_A2B.predict(real_A, batch_size=4)
            self.save_tmp_images(real_A, real_B, synthetic_image_A, synthetic_image_B)

        self.saveModel(self.G_A2B, 150)
        self.saveModel(self.G_B2A, 150)

#===============================================================================
# Training
    def train(self, epochs, batch_size=4, save_interval=1):
...
# Help functions
    ...
    def cycle_loss(self, y_true, y_pred):
        l = 0.99
        c_loss = tf.reduce_mean(tf.abs(y_pred - y_true))
        loss = l*c_loss + (1-l)*self.gram_loss(y_true,y_pred)
        return loss

    def gram_loss(self, y_true, y_pred):
        def gram_matrix(x):
            #assert K.ndim(x) == 3
...
#        assert K.ndim(y_pred) == 3
        ...
        channels = 1
        size = 240 * 240
        gm_loss = K.sum(K.square(S - C)) / (4.0 * (channels ** 2) * (size ** 2))
        return gm_loss
